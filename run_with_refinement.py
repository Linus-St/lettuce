...
def together(fine: np.array, coarse: np.array, offset):
    ...
    print("x_slice", area_fine_x)
    print("y_slice", area_fine_y)
    print(coarse.shape)
    print(result[area_fine_x, area_fine_y].shape)
...
flow_fine = lt.Obstacle(context_fine, [res_fine_x, res_fine_y], reynolds_number=reynolds_fine,
                          mach_number=mach_fine, domain_length_x=length_fine, char_length=0.5)
...
simulation_coarse = Simulation(flow=flow_coarse, collision=collision_coarse, reporter=[], child_simulation=simulation_fine, child_offset=[x_min_coarse, y_min_coarse])
